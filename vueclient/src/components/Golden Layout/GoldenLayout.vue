<template>
	<!--—амый внешний контейнер позиционируетс€ относительно, что гарантирует, что все дочерние элементы могут использовать абсолютное позиционирование относительно этого контейнера.-->
	<div style="position: relative">
		<SlotExtr ref="predef">
			<!-- если существует свойство "router", то... кака€ то шл€па €вно происходит-->
		...
		<!-- отображение всех (v-for) компонентов по.. внутри шаблона gl-template, чтобы это ни означало -->
		<div style="position: absolute; width: 100%; height: 100%">
			<gl-template v-for="pair in AllComponents"
						 :key="pair[0]"
...
<script setup lang="ts"> //будет TypeScript
import {
	onMounted, //событие при монтированни (кажетс€, когда компонент попадает из виртуального на реальное dom дерево?)
	ref,       //ссылочка vue
	readonly,  //только дл€ чтени€, прокси
    nextTick,  // A utility for waiting for the next DOM update flush. function nextTick(callback?: () => void): Promise<void>
	getCurrentInstance, //получить внутренний инстанс vue3 дл€ компонента.
	provide,            //родительский компонент имеет опцию provide дл€ предоставлени€ данных, а дочерний компонент имеет опцию inject дл€ внедрени€ и использовани€ этих данных
	type PropType,      //чтобы в defineComponent можно было вставл€ть сложный объект, типизированный
	type Slots          
} from "vue";
import {
	ComponentContainer,
...
import SlotExtr from "./SlotExtr.vue";
import FixedRoute from './FixedRoute.vue';
 ...
 * Data
 *******************/
	GLRoot = ref<null | HTMLElement>(null), //ссылочка на корневой элемент дл€ GL
	GlcKeyPrefix = readonly(ref("glc_")), //текстовый префикс дл€ названий компонентов. почему то завЄрнутый в ref
	MapComponents = new Map<  //ни что иное, как словарь. использовать get/set методы
		ComponentContainer,
		{ refId: number; glc: typeof GlTemplate }
	>(),
	AllComponents = ref(new Map<number, any>()), // реактивна€ ссылочка на коллекцию компонентов
